...
makemove (time, captured, board, (x:xs))
	| blackKilled = makemove (restartTime, restartCaptured, restartBoard, xs)
	| otherwise = makemove (newTime, newCaptured, newBoard, xs)
	where
		(newTime, newCaptured, newBoard) = trymove (time, captured, board, x)
		blackKilled = kingKilled newCaptured
		(restartTime, restartCaptured, restartBoard) = restart (time, captured, board)
